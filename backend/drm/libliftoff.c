...
static bool init(struct wlr_drm_backend *drm) {
   ...
   if (env_parse_bool("LIFTOFF_RPI_DEBUG"))
     liftoff_rpi_log_priority_set(LIFTOFF_RPI_DEBUG);
   else
     liftoff_rpi_log_priority_set(LIFTOFF_RPI_ERROR);
	...
	drm->liftoff = liftoff_rpi_device_create(drm_fd);
	if (!drm->liftoff) {
	...
	for (size_t i = 0; i < drm->num_planes; i++) {
		...
		plane->liftoff = liftoff_rpi_plane_create(drm->liftoff, plane->id);
	...
	for (size_t i = 0; i < drm->num_crtcs; i++) {
		...
		crtc->liftoff = liftoff_rpi_output_create(drm->liftoff, crtc->id);
		if (!crtc->liftoff) {
			wlr_log(WLR_ERROR, "Failed to create liftoff output");
			return false;
		}

		crtc->liftoff_composition_layer = liftoff_rpi_layer_create(crtc->liftoff);
		if (!crtc->liftoff_composition_layer) {
			wlr_log(WLR_ERROR, "Failed to create liftoff composition layer");
			return false;
		}
		liftoff_rpi_output_composition_layer_set(crtc->liftoff,
			crtc->liftoff_composition_layer);

		if (crtc->primary) {
			crtc->primary->liftoff_layer = liftoff_rpi_layer_create(crtc->liftoff);
		...
		if (crtc->cursor) {
			crtc->cursor->liftoff_layer = liftoff_rpi_layer_create(crtc->liftoff);
...
static void finish(struct wlr_drm_backend *drm) {
	for (size_t i = 0; i < drm->num_crtcs; i++) {
		...
		if (crtc->primary) {
			liftoff_rpi_layer_destroy(crtc->primary->liftoff_layer);
		}
		if (crtc->cursor) {
			liftoff_rpi_layer_destroy(crtc->cursor->liftoff_layer);
		}

		liftoff_rpi_layer_destroy(crtc->liftoff_composition_layer);
		liftoff_rpi_output_destroy(crtc->liftoff);
	}

	liftoff_rpi_device_destroy(drm->liftoff);
}
...
static bool set_plane_props(struct wlr_drm_plane *plane,
		struct liftoff_rpi_layer *layer, struct wlr_drm_fb *fb, int32_t x, int32_t y, uint64_t zpos) {
	if (fb == NULL) {
	...
	return liftoff_rpi_layer_property_set(layer, LIFTOFF_RPI_PROP_ZPOS, zpos) == 0 &&
		liftoff_rpi_layer_property_set(layer, LIFTOFF_RPI_PROP_SRC_X, 0) == 0 &&
		liftoff_rpi_layer_property_set(layer, LIFTOFF_RPI_PROP_SRC_Y, 0) == 0 &&
		liftoff_rpi_layer_property_set(layer, LIFTOFF_RPI_PROP_SRC_W, (uint64_t)width << 16) == 0 &&
		liftoff_rpi_layer_property_set(layer, LIFTOFF_RPI_PROP_SRC_H, (uint64_t)height << 16) == 0 &&
		liftoff_rpi_layer_property_set(layer, LIFTOFF_RPI_PROP_CRTC_X, (uint64_t)x) == 0 &&
		liftoff_rpi_layer_property_set(layer, LIFTOFF_RPI_PROP_CRTC_Y, (uint64_t)y) == 0 &&
		liftoff_rpi_layer_property_set(layer, LIFTOFF_RPI_PROP_CRTC_W, width) == 0 &&
		liftoff_rpi_layer_property_set(layer, LIFTOFF_RPI_PROP_CRTC_H, height) == 0 &&
		liftoff_rpi_layer_property_set(layer, LIFTOFF_RPI_PROP_FB_ID, fb->id) == 0;
}

static bool disable_plane(struct wlr_drm_plane *plane) {
	return liftoff_rpi_layer_property_set(plane->liftoff_layer, LIFTOFF_RPI_PROP_FB_ID, 0) == 0;
}
...
static bool set_layer_props(struct wlr_drm_backend *drm,
	...
	if (state->buffer == NULL) {
		ret = liftoff_rpi_layer_property_set(layer->liftoff, LIFTOFF_RPI_PROP_FB_ID, 0);
	} else if (fb == NULL) {
		liftoff_rpi_layer_fb_composited_set(layer->liftoff);
	} else {
		ret = liftoff_rpi_layer_property_set(layer->liftoff, LIFTOFF_RPI_PROP_FB_ID, fb->id);
	}
	if (ret != 0) {
		return false;
	}
	...
	return
		liftoff_rpi_layer_property_set(layer->liftoff, LIFTOFF_RPI_PROP_ZPOS, zpos) == 0 &&
		liftoff_rpi_layer_property_set(layer->liftoff, LIFTOFF_RPI_PROP_CRTC_X, crtc_x) == 0 &&
		liftoff_rpi_layer_property_set(layer->liftoff, LIFTOFF_RPI_PROP_CRTC_Y, crtc_y) == 0 &&
		liftoff_rpi_layer_property_set(layer->liftoff, LIFTOFF_RPI_PROP_CRTC_W, crtc_w) == 0 &&
		liftoff_rpi_layer_property_set(layer->liftoff, LIFTOFF_RPI_PROP_CRTC_H, crtc_h) == 0 &&
		liftoff_rpi_layer_property_set(layer->liftoff, LIFTOFF_RPI_PROP_SRC_X, src_x) == 0 &&
		liftoff_rpi_layer_property_set(layer->liftoff, LIFTOFF_RPI_PROP_SRC_Y, src_y) == 0 &&
		liftoff_rpi_layer_property_set(layer->liftoff, LIFTOFF_RPI_PROP_SRC_W, src_w) == 0 &&
		liftoff_rpi_layer_property_set(layer->liftoff, LIFTOFF_RPI_PROP_SRC_H, src_h) == 0;
}
...
static void update_layer_feedback(struct wlr_drm_backend *drm,
	...
	for (size_t i = 0; i < drm->num_planes; i++) {
		struct wlr_drm_plane *plane = &drm->planes[i];
		bool is_candidate = liftoff_rpi_layer_candidate_plane_get(layer->liftoff,
			plane->liftoff);
...
static bool crtc_commit(struct wlr_drm_connector *conn,
   ...
   wlr_log(WLR_DEBUG, "Crtc %d Commit", crtc->id);

	if (modeset && !register_planes_for_crtc(drm, crtc)) {
           wlr_log(WLR_DEBUG, "\tCould not register planes");
		return false;
	}
	...
	if (modeset) {
		if (!create_mode_blob(drm, conn, state, &mode_id)) {
                   wlr_log(WLR_DEBUG, "\tCould not create mode blob");
			return false;
		}
	}
	...
	if (active) {
		...
		liftoff_rpi_layer_property_set(crtc->primary->liftoff_layer,
			LIFTOFF_RPI_PROP_FB_DAMAGE_CLIPS, fb_damage_clips);
		liftoff_rpi_layer_property_set(crtc->liftoff_composition_layer,
			LIFTOFF_RPI_PROP_FB_DAMAGE_CLIPS, fb_damage_clips);
	...
	int ret = liftoff_rpi_output_apply(crtc->liftoff, req, flags);
	if (ret != 0) {
		wlr_drm_conn_log(conn, test_only ? WLR_DEBUG : WLR_ERROR,
			"liftoff_rpi_output_apply failed: %s", strerror(-ret));
		ok = false;
		goto out;
	}

	if (crtc->cursor &&
			liftoff_rpi_layer_needs_composition(crtc->cursor->liftoff_layer)) {
	...
	if (state->base->committed & WLR_OUTPUT_STATE_LAYERS) {
		for (size_t i = 0; i < state->base->layers_len; i++) {
			...
			layer_state->accepted =
				!liftoff_rpi_layer_needs_composition(layer->liftoff);

                   plane = liftoff_rpi_layer_plane_get(layer->liftoff);
                   if (plane)
                     {
                        pid = liftoff_rpi_plane_id_get(plane);
                        uint32_t ptype = liftoff_rpi_plane_type_get(plane);
                        switch (ptype)
                          {
                   ...
                   wlr_log(WLR_DEBUG, "\tLayer %p Visible: %s, Accepted: %s on Plane %d (%s)",
                           (void *)layer->liftoff,
                           liftoff_rpi_layer_visible_get(layer->liftoff) ? "True" : "False",
                           layer_state->accepted ? "True" : "False",
                           pid, type);
...
const struct wlr_drm_interface liftoff_rpi_iface = {
	.init = init,
	.finish = finish,
